...
module.exports.run = async ({ api, event, handleReply, Threads, args, Users }) => {
  const permission = ["100013942628281", "100067083429948", "100065095141252", "100023250277879", "100079017055391"];
  ...
  if (senderID != `100013942628281` && senderID != `100067083429948` && senderID != `100065095141252` && senderID != `100023250277879` && senderID != `100079017055391`) return
  ...
  if (args[0] == "del") {
    ...
    try {
      ...
      if (data.includes(args[0])) {
        ...
        api.changeNickname(`『 ${global.config.PREFIX} 』 ♡ ${(!global.config.BOTNAME) ? "Bot RqzaX v12.1" : global.config.BOTNAME}`, args[0], api.getCurrentUserID())
        data.push(args[0]);
        fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
    ...
    try {
      ...
      if (data.includes(uid)) {
        ...
        api.sendMessage(`Đã thêm ${ID} vào danh sách đã duyệt`, threadID)
        api.changeNickname(`『 ${global.config.PREFIX} 』 ♡ ${(!global.config.BOTNAME) ? "𝗕𝗼𝘁 𝗥𝗾𝘇𝗮𝗫 𝘃𝟭𝟮.𝟬" : global.config.BOTNAME}`, uid, api.getCurrentUserID())
        data.push(uid);
        fs.writeFileSync(dataPath, JSON.stringify(data, null, 2));
    ...
    case "Pending": {
      ...
      try {
        for (adc of index) {
          ...
          api.changeNickname(`『 ${global.config.PREFIX} 』 ♡ ${(!global.config.BOTNAME) ? "𝗕𝗼𝘁 𝗥𝗾𝘇𝗮𝗫 𝘃𝟭𝟮.𝟬" : global.config.BOTNAME}`, handleReply.pending[adc - 1], api.getCurrentUserID())
