...
module.exports.handleEvent = async ({ event, api, Users }) => {
  let AutoUnSend = 120;// theo giây
  let { get } = require("axios");
  let { threadID, body, senderID } = event;
  let r = await get('https://api-web.miraiofficials123.repl.co/');
  let m = r.data.url;
  let cc = (await get(m, {
			responseType: "stream"
		})).data;
  ...
  switch (args[0]) {
    ...
    case 'on': {
      if (config.autoSend == false) {
        ...
        api.sendMessage(`Bật thành công gửi lời chào đến thành viên`, threadID);
      }
    ...
    case 'off': {
      if (config.autoSend == true) { 
        ...
        api.sendMessage(`Tắt thành công gửi lời chào đến thành viên`, threadID);
      }
      fs.writeFileSync(configPath, JSON.stringify(config, null, 4), 'utf8');
      break;
    }
